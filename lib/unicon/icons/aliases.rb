module Unicon
  # Model names a host app may look up, each naming the concept it takes its icons from
  ALIASES = {
    address: :pin, advice: :reply, agent: :badge, alert: :warning, announcement: :megaphone,
    answer: :question, api: :terminal, app: :window, appointment: :calendar_check,
    area: :map, assessment: :clipboard, attachment: :paperclip, audio: :waveform,
    author: :account, avatar: :account, backup: :cloud, band: :tag, banner: :image, bill: :invoice,
    booking: :calendar_check, boost: :bolt, business: :building, call: :phone,
    campaign: :megaphone, category: :tag, charge: :credit_card, city: :building,
    client: :person, comment: :chat, company: :building, configuration: :gear,
    contract: :signature, conversation: :chat, cost: :cash, country: :globe,
    county: :map, coupon: :gift, credential: :key, customer: :person,
    dashboard: :speedometer, deadline: :alarm, delivery: :truck, destination: :pin,
    discount: :percent, echo: :waveform, email: :envelope, employee: :badge,
    enrichment: :document, episode: :playlist, evaluation: :speedometer, event: :calendar,
    export: :download,
    feedback: :smiley, file: :document, fleet: :truck, form: :document,
    goal: :trophy, group: :people, history: :clock,
    home: :house, import: :upload, inquiry: :wand, integration: :puzzle,
    inventory: :box, issue: :warning, item: :box, job: :hammer, label: :tag,
    lead: :person, line: :box, location: :pin, log: :list, market: :shop,
    member: :person, memo: :note, message: :chat, milestone: :flag,
    movie: :video, nomination: :gift, notification: :bell, offer: :gift, office: :building,
    opening: :stopwatch,
    optimization: :sliders,
    order: :cart, organization: :building, package: :box, page: :document,
    password: :lock, payment: :credit_card, permission: :key, photo: :image,
    pick: :checklist, picture: :image, plan: :calendar, platform: :puzzle, plugin: :puzzle,
    post: :document, prediction: :wand, preference: :sliders, price: :tag, product: :box,
    profile: :account, project: :folder, prompt: :terminal, provider: :briefcase,
    purchase: :cart, rating: :star, reading: :wand, recording: :microphone, region: :map,
    reminder: :bell, review: :star, ride: :car, role: :badge, route: :signpost,
    schedule: :calendar, screen: :display, sector: :pie_chart, service: :wrench, session: :clock,
    setting: :gear,
    shift: :clock, shipment: :truck, site: :globe, skill: :star, source: :signpost,
    specialty: :award, staff: :people, state: :map, subscription: :repeat,
    summary: :document, supplier: :truck, survey: :clipboard, sync: :refresh,
    team: :people, technician: :toolbox, territory: :map, todo: :task, token: :key,
    track: :music, transaction: :credit_card, trip: :map, user: :person,
    vehicle: :car, vendor: :shop, vertical: :bar_chart, view: :eye, visit: :clock,
    zip: :pin,
  }.freeze
end
